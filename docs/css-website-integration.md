...
If you have a button that uses an icon instead of text the icon won’t appear in print because images are hidden. This could result in an empty-looking button. In this case, you can add a print-only text label to ensure the printed output still makes sense.
```html
<button>
  <img …>
  <span class="print-only">Submit</span>
</button>
```


## Including your organisations's logo
